...
This package is designed for calculating various metrics from
...
1.  `lidar_SHM` calculates a shrub height model (SHM)
    from lidar point cloud data. The function takes a `LAS` or `LAScatalog`
    object and returns an SHM raster in `.tif` format. The key feature in
    the function is (1) the stretching of point clouds into "pseudo-forests"
    to prevent shrub points from being misclassified as ground and
    (2) normalising points by interpolating points in a coarser DTM to prevent
    misclassified ground points from erasing shrubs. The default parameters
    were optimised for an airborne LiDAR scan (22 pts/m2) of the short Ericaceous 
    shrub layer (mean 0.36 m in height) in Cairngorms Connect, Scotland.
    Please note that, like any lidar-based canopy height estimation, SHM tend to
    systematically underestimate heights, so corrections based on field data is
    recommended if the heights need to be taken at face value. The manuscript
    for the SHM pipeline is currently under review, so at the meantime, please
    cite the preprint (Chan et al., [2025](https://dx.doi.org/10.2139/ssrn.5527379)).
...
2.  `CHM_metrics` calculates a number of useful structural metrics from
    ...
    Zhang et al. ([2024](https://doi.org/10.1002/rse2.398)) with several minor modifications and additions (see table below).
...
3.  `liDAR_metrics` calculates a number of useful structural metrics
    ...
    al. ([2023](https://doi.org/10.1016/j.rse.2022.113326)).
...
function.
Jean-Romain Roussel - for providing support on the `lidR` package which the `lidarSHM` is built on.
